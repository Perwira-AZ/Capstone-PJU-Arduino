...
const int ldrEnv = 12;  // LDR sensor to detect the environment light
const int ldr1 = 13;
const int led1 = 27;
...
void loop() {
  ...
  if (packetSize) {
    ...
    if (received.startsWith("PJU-1-" + block)) {
      if (received.endsWith("-I")) {
        resetArray();
        delay(500);
        sendLoRa("PJU-3-" + block + "-I", "Forward to node");  // Forwarding message to all node in the block
        command = "I";
        ldrValues[0] = analogRead(ldr1);
        nodesStatus[0] = (digitalRead(led1) == HIGH) ? 1 : 0;
        delay(500);
        setResponseCode(0, (analogRead(ldr1) >= ledThreshold) ? 1 : 0, analogRead(ldr1));
        lastSendTime = millis();
        ...
        lastSendTime = millis();
        waitingForResponse = true;
      } else if (received.endsWith("-S")) {  // Receiving reply from the node
